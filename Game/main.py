        ...
        if current_button.collidepoint ((mx, my)):
            if click:
                if map.current_encounter in {0, 1, 3, 4, 6, 7, 9, 10}:
                    if battle_sequence() == False:
                        run = False
                        screen = pygame.display.set_mode((screen_width, screen_height - bottom_panel))
                        map.current_encounter = 0
                if map.current_encounter in {2, 5, 8}:
                    event_sequence(encounters[map.current_encounter]["string_list"])
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            #press SPACE to fullscreen
                ...
                screen = pygame.display.set_mode((screen_width, screen_height), pygame.FULLSCREEN|HWSURFACE)
                pygame.display.flip()
